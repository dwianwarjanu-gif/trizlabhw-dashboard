...
describe('OrderStatusBadge', () => {
    ...
    it('renders DELIVERED status correctly', () => {
        render(_jsx(OrderStatusBadge, { status: "DELIVERED" }));
        const badge = screen.getByText('Selesai');
        expect(badge).toBeInTheDocument();
        expect(badge).toHaveClass('bg-green-100', 'text-green-800');
    });
    it('renders CANCELLED status correctly', () => {
        render(_jsx(OrderStatusBadge, { status: "CANCELLED" }));
        const badge = screen.getByText('Dibatalkan');
        expect(badge).toBeInTheDocument();
        expect(badge).toHaveClass('bg-red-100', 'text-red-800');
    });
    it('renders REFUNDED status correctly', () => {
        render(_jsx(OrderStatusBadge, { status: "REFUNDED" }));
        const badge = screen.getByText('Dikembalikan');
        expect(badge).toBeInTheDocument();
        expect(badge).toHaveClass('bg-gray-100', 'text-gray-800');
    });
    it('renders unknown status as default', () => {
        render(_jsx(OrderStatusBadge, { status: "UNKNOWN_STATUS" }));
        const badge = screen.getByText('UNKNOWN_STATUS');
        expect(badge).toBeInTheDocument();
        expect(badge).toHaveClass('bg-gray-100', 'text-gray-800');
    });
    it('applies small size correctly', () => {
        render(_jsx(OrderStatusBadge, { status: "PENDING", size: "sm" }));
        const badge = screen.getByText('Menunggu');
        expect(badge).toHaveClass('px-2', 'py-0.5', 'text-xs');
    });
    it('applies large size correctly', () => {
        render(_jsx(OrderStatusBadge, { status: "PENDING", size: "lg" }));
        const badge = screen.getByText('Menunggu');
    ...
    it('renders with custom className', () => {
        render(_jsx(OrderStatusBadge, { status: "PENDING", className: "custom-class" }));
        const badge = screen.getByText('Menunggu');
    ...
    describe('Status mapping', () => {
        ...
        statusMappings.forEach(({ status, label, color }) => {
            it(`maps ${status} to ${label} with ${color} color`, () => {
                render(_jsx(OrderStatusBadge, { status: status }));
    ...
    describe('Edge cases', () => {
        it('handles empty status', () => {
            render(_jsx(OrderStatusBadge, { status: "" }));
            const badge = screen.getByRole('status');
            expect(badge).toBeInTheDocument();
            expect(badge).toHaveTextContent('');
        });
        it('handles null status', () => {
            render(_jsx(OrderStatusBadge, { status: null }));
            const badge = screen.getByRole('status');
            expect(badge).toBeInTheDocument();
        });
        it('handles undefined status', () => {
            render(_jsx(OrderStatusBadge, { status: undefined }));
            const badge = screen.getByRole('status');
            expect(badge).toBeInTheDocument();
        });
    });
...
//# sourceMappingURL=OrderStatusBadge.test.js.map